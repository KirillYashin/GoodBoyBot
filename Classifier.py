...
def list_classifier(to_classify: list):
    #model = 'public\\se-resnet-50\\FP32\\se-resnet-50.xml'
    #weights = 'public\\se-resnet-50\\FP32\\se-resnet-50.bin'
    model = 'public\\resnet-50-tf\\FP16\\resnet-50-tf.xml'
    weights = 'public\\resnet-50-tf\\FP16\\resnet-50-tf.bin'
    classes_path = 'imagenet_synset_words.txt'
    device = 'CPU'
    cpu_extension = 'CPU'
    ...
    for img in to_classify:
        ...
        predictions = [str(ie_classifier.labels_map[predictions[i]]) + ': '
                       + str(predictions[i]) for i in range(top_n)]
